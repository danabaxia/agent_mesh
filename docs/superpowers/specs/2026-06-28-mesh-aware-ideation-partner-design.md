...
- **Proactivity is reactive (no unsolicited audio, no fake "session start"):** the concierge only runs *in response to a user utterance* — there is no session-start event in the A2A lifecycle. The proactive entry point is: when the owner's turn is an **open-ended opener** (greeting / "what's up" / "anything I should think about?"), the concierge calls `brainstorm_seeds` and offers **one** spark as part of that reply. "First turn of a session" is derived **JS-side, with no ingress change**, from per-session conversation history (`src/brains/history-store.js`) — an **empty history ⇒ first turn**. **Prerequisite (small, data-only JS change):** history is currently keyed by `session.id` (`_anon` over stdio, absent over HTTP), which is **not** per-phone-session. This feature **threads `message.contextId`** — which the voice ingress already stamps per session — from the A2A `SendMessage` through both `runAgent` transports (`src/a2a/stdio-server.js` + `http-server.js`) into `runGeminiAgent`, using it as the history/session key (falling back to the old key only when `contextId` is absent). `runGeminiAgent` then passes a `firstTurn` boolean (empty history for this `contextId`) into the brain's turn context; the prompt says *on `firstTurn`, you MAY open with one spark*. It never speaks unprompted, and the **Python ingress is untouched**. **Tests (both transports):** a `SendMessage` with a fresh `contextId` ⇒ empty history ⇒ `firstTurn` true and `brainstorm_seeds` considered; a second `SendMessage` with the same `contextId` ⇒ history present ⇒ `firstTurn` false, no auto-open; over **stdio and HTTP** the `contextId` reaches the history key.
...
- **Component C:** `brainstorm_seeds` default backend over a temp digest file (returns seeds; `topic` filter; empty/missing → `{seeds:[]}`; surfaces `degraded`; degrade-not-throw). Consistent with the existing `brains-tools.test.js` default-backend tests.
  - **firstTurn + contextId-keying test (both transports):** a `SendMessage` with a fresh `contextId` ⇒ empty history ⇒ `firstTurn` true and `brainstorm_seeds` considered; same `contextId` again ⇒ history present ⇒ false, no auto-open; assert `contextId` reaches the history key over **stdio and HTTP**. Pure JS, no ingress involvement.
...
- [MAJOR] `firstTurn` metadata stamping in the Python ingress contradicted "No Python changes" → replaced with **JS-side derivation**: empty per-session history ⇒ first turn. The Python ingress is untouched; the contract holds.
