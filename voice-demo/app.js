...
// Stable session id (persists across reload / iOS suspend) → the server keeps the
// conversation memory keyed by this, so the assistant never "forgets" on refresh.
const SESSION = (() => {
  try {
    let s = localStorage.getItem('voice_session');
    if (!s) { s = (crypto.randomUUID ? crypto.randomUUID() : String(Date.now()) + Math.random()); localStorage.setItem('voice_session', s); }
    return s;
...
async function send(text) {
  ...
  try {
    ...
    const res = await fetch(api('chat'), { method: 'POST', headers: authHdr({ 'Content-Type': 'application/json' }), body: JSON.stringify({ history: convo.slice(0, -1), text, confirmBeforeFile, session: SESSION }) });
    clearInterval(beat);
...
// Re-hydrate the conversation from the server (survives reload / iOS suspend), so
// you see prior turns and the assistant keeps its memory.
async function hydrateHistory() {
  try {
    const r = await fetch(api('history?session=' + encodeURIComponent(SESSION)), { headers: authHdr() }).then((x) => x.json());
    if (r && r.ok && Array.isArray(r.turns) && r.turns.length) {
      for (const t of r.turns) { bubble(t.role === 'assistant' ? 'ai' : 'me', t.text || ''); convo.push({ role: t.role, text: t.text || '' }); }
      return true;
    }
  } catch {}
  return false;
}
loadVoices().then(hydrateHistory).then((had) => {
  if (!had) bubble('ai', '你好，我是你的 mesh 助手。点「🚗 连续对话」免提聊（开车也能用），或按麦克风/打字。把好想法随口说给我，我帮你记下来、安排成任务。');
});
